...
case "$NODE_OS" in
  ...
  ubuntu)
    echo "===> Creating a CloudFormation stack in AWS named k8s-tests-$CLUSTER_ID"
    aws cloudformation create-stack --stack-name k8s-tests-$CLUSTER_ID --template-body file://aws-cloudformation/6-nodes-ubuntu-18-04-v2.json --parameters ParameterKey=SSHKey,ParameterValue=aws_demo_sales_new ParameterKey=TestClusterID,ParameterValue=$CLUSTER_ID
    ;;
  *)
